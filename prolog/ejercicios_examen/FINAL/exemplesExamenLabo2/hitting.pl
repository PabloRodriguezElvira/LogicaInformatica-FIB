...
main:- findall( C, (word(List), member(C, List)), Chars0),
       sort(Chars0, CharsSorted),   % sort to avoid repetitions
       between(1,1000,Size),
       subsetOfSize(Size, CharsSorted, Subset),
       allWordsCovered(Subset),
       write(Subset), nl, halt.
...
wordCovered(W,L):- member(Char, W), member(Char, L), !.
