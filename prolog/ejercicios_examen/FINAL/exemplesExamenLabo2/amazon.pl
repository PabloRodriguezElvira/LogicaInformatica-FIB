...
eachDayEnoughWorkers:- 
    day(Day), numWorkersNeededForDay(Day, NumWorkers), 
    findall(works(Worker, Day), worker(Worker), Lits),
    atLeast(NumWorkers, Lits), fail.
eachDayEnoughWorkers.

%isBlocked(Worker, Day) ==> !works(Worker, Day)
blocked:- 
    worker(Worker), findall(works(Worker, Day), isBlocked(Worker, Day), Lits), exactly(0, Lits), fail. 
blocked.

atMostThreeConsecutiveDays:- 
    fourConsecutiveDays(D1, D2, D3, D4), worker(Worker), 
    writeClause([-works(Worker, D1), -works(Worker, D2), -works(Worker, D3), -works(Worker, D4)]), fail.
atMostThreeConsecutiveDays.

defineNoSupervisor:-
    day(D), findall(works(Worker, D), supervisor(Worker), Lits), 
    negateAll(Lits, NegatedLits), expressAnd(noSupervisor(D), NegatedLits), fail.
defineNoSupervisor.

maxDaysWithoutSupervis(Max):- findall(noSupervisor(Day), day(Day), Lits), atMost(Max, Lits), fail.
...
%
costOfThisSolution(M,Cost):- findall(Day, member(noSupervisor(Day), M), Lits), length(Lits, Cost), !.
% maxDaysWithoutSupervisor(Max), between(0, Max, I), Cost is Max-I,
