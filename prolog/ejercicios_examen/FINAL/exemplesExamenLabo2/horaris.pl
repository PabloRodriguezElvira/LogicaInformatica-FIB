...
%% A university wants us to design a weekly schedule of a certain
...
eachCourseExactlyOneRoom:- courseRooms(C, Rooms), findall(cr(C, R), member(R, Rooms), Lits), exactly(1, Lits), fail. 
eachCourseExactlyOneRoom.

eachCourseMaxOncePerDay:- course(C), day(D), findall(cdh(C, D, H), hour(H), Lits), atMost(1, Lits), fail.
eachCourseMaxOncePerDay.

% cd(C, D) <===> cdh(C, D, 0) V cdh(C, D, 1) V ... V cdh(C, D, 19)
relateVarsCDHwithCD:- course(C), day(D), findall(cdh(C, D, H), hour(H), Lits), expressOr(cd(C,D),Lits), fail.
relateVarsCDHwithCD.

eachCourseRightNumberOfDays:- courseHours(C, NumOfDays), findall(cd(C, D), day(D), Lits), exactly(NumOfDays, Lits), fail.
eachCourseRightNumberOfDays.

% cdh(C1, D, H) ===> -cdh(C2, D, H) si C1 y C2 son del mismo año.
noOverlapCoursesSameYear:- courseYear(C1, Y), courseYear(C2, Y), C1 \= C2, day(D), hour(H), writeClause([-cdh(C1, D, H), -cdh(C2, D, H)]), fail.
noOverlapCoursesSameYear.

% cdh(C1, D, H) ^ cdh(C2, D, H) ^ cp(C1, P) ===> -cp(C2, P)
noOverlapProfs:- courseProfessors(C1, ListaProfesC1), courseProfessors(C2, ListaProfesC2), C1 \= C2, 
    member(P, ListaProfesC1), member(P, ListaProfesC2), day(D), hour(H), writeClause([ -cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P) ]), fail. 
noOverlapProfs.

% cdh(C1, D, H) ^ cdh(C2, D, H) ^ cr(C1, R) ===> -cr(C2, R)
noOverlapRooms:- courseRooms(C1, ListaRoomsC1), courseRooms(C2, ListaRoomsC2), C1 \= C2, 
    member(R, ListaRoomsC1), member(R, ListaRoomsC2), day(D), hour(H), writeClause([-cdh(C1, D, H), -cdh(C2, D, H), -cr(C1, R), -cr(C2, R)]), fail.
noOverlapRooms.

% cp(C, Prof) ===> usedProf(Prof)
relateVarsCPwithUsedProf:- courseProfessors(C, ListaProfs), member(Prof, ListaProfs), writeClause([-cp(C, Prof), usedProf(Prof)]), fail.
relateVarsCPwithUsedProf.
