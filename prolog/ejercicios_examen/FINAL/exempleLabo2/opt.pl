...
maxTwoDoublesPerTeam:- team(S), findall(double(S, R), (round(R), R > 1), Lits), atMost(2, Lits), fail.
...
% double(Team, R) ^ double(Team, R1) ===> hasTwoDoubles(Team). 
twoDoublesImpliesHasTwoDoubles:- team(Team), round(R), round(R1), R \= R1, writeClause([-double(Team, R), -double(Team, R1), hasTwoDoubles(Team)]), fail.
...
% solution of this exam:
costOfThisSolution(M,Cost):- between(0, 20, I), Cost is 20-I, findall(Team, member(hasTwoDoubles(Team), M), L), length(L, Cost), !.
