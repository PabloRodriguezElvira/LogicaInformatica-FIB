...
%verifies that double(Team, R) <---> (home(Team, R) & home(Team, R-1)) V (-home(Team, R) & -home(Team, R-1)).
doubleConstraint:-
    team(Team),
    round(R),
    R \= 1,
...
sixOrSevenHomes:-  
    team(Team),
    findall(home(Team, R), round(R), Lits),
    atLeast(6, Lits),
    atMost(7, Lits),
    fail.
sixOrSevenHomes.

%No team gets more than one double in the whole league
atMostOneDouble:-  
    team(Team),
    findall(double(Team, R), round(R), Lits),
    atMost(1, Lits),
    fail.
atMostOneDouble.
