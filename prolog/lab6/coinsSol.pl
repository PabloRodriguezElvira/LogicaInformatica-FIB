...
main:- 
    ...
    length(Coins,N), 
    length(Vars, N), % get list of N prolog vars. N variables de prolog sin valor.

%1: Dominio:
    Vars ins 0..Amount,   % en el peor de los casos, solo hay monedas de valor 1. Se pueden definir dominios diferentes para cada variable.

%2: Constraints:
    expr( Vars, Coins, Expr ),  % por ejemplo expr( [X1,X2,X3,X4], [1,2,5,10],   X4*10 + X3*5 + X2*2 + X1*1 )
    Expr #= Amount, % #= significa que hay un constraint de igualdad (# es para distinguir del = de Prolog).
    exprSuma( Vars, ExprSum ),  % por ejemplo exprSuma( [X1,X2,X3,X4], X4+X3+X2+X1 )

%3: labeling:
    labeling( [min(ExprSum)], Vars  ), % qué variables quiero que se instancien.  labeling(estrategia, variables). estrategia de minimizar la suma.  
    % Queremos minimizar la cantidad de monedas usadas.
