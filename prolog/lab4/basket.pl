...
writeClauses(MaxCost):-
    ...
    teamCannotPlayAtHome,
    eachRoundOneMatchtvTeams,
    doubleConstraint,
    noTriplets,
    maxCost(MaxCost),
    true,!.
...
homesAndAways:-
    difTeams(S, T), round(R),
    writeClause( [-match(S, T, R), home(S, R)]),
    writeClause( [-match(S, T, R), -home(T, R)]),
    fail.
homesAndAways.

%double -> no 2 consecutive matches away neither home.
%home(Team, R) ^ home(Team, R1) --> double(Team, R).
%-home(Team, R) ^ -home(Team, R1) --> double(Team, R) 
doubleConstraint:-
    team(Team),
    round(R),
    R \= 1,
    R1 is R-1,
    writeClause([-home(Team, R), -home(Team, R1), double(Team, R)]), 
    writeClause([home(Team, R), home(Team, R1), double(Team, R)]),
    writeClause([-home(Team, R), home(Team, R1), -double(Team, R)]),
    writeClause([home(Team, R), -home(Team, R1), -double(Team, R)]),
    fail.
...
noTriplets:-
    ...
    R \= 1,
    R1 is R-1,
    writeClause( [-double(Team, R), -double(Team, R1)]),
    fail.
noTriplets.
