...
satVariable( tasktime(T, H) ):- task(T), integer(H). % "task T is executed during hour H."
...
writeClauses(MaxHours):-
    ...
    defineTaskTime(MaxHours),
    resourcesDontOverlap(MaxHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachTaskStartsOnce(MaxHours):-
    duration(Task, Duration),
    MaxStart is MaxHours - Duration + 1,
    findall(start(Task, Hour), between(1, MaxStart, Hour), Lits),
    exactly(1, Lits),
    fail.
eachTaskStartsOnce(_).

% start(Task, Start) <---> tasktime(Task, H1) ^ tasktime(Task, H2) ^ ... tasktime(Task, Hend).
defineTaskTime(MaxHours):-
    duration(Task, Duration),
    MaxStart is MaxHours - Duration + 1,
    between(1, MaxStart, Start),
    End is Start + Duration - 1,
    findall(tasktime(Task, Hour), between(Start, End, Hour), Lits),
    expressAnd(start(Task, Start), Lits), % <--->
    fail.
defineTaskTime(_).

resourcesDontOverlap(MaxHours):-
    resourceUnits(Resource, Units),
    between(1, MaxHours, Hour),
    findall(tasktime(Task, Hour), usesResource(Task, Resource), Lits),
    atMost(Units, Lits),
    fail.
resourcesDontOverlap(_).
