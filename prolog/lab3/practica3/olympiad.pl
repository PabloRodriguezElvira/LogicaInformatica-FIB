...
mutuallyFriends(S1, S2):- friends(S1, S2), friends(S2, S1).
...
writeClauses:- 
    teamWithExactlyKMembers,
    allDifferentExpertise,
    allMutuallyFriends,
    true, !.
...
teamWithExactlyKMembers:-
    numMembersTeam(K), findall(chosen(S), student(S), Lits), exactly(K, Lits), fail.
teamWithExactlyKMembers.


allDifferentExpertise:-
    expertise(_, E), findall(chosen(S), expertise(S, E), Lits), atMost(1, Lits), fail.
allDifferentExpertise.


allMutuallyFriends:-
    student(S1), student(S2), S1 \= S2, not(mutuallyFriends(S1, S2)), atMost(1, [chosen(S1), chosen(S2)]), fail.
allMutuallyFriends.
