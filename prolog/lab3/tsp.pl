...
satVariable( cost(P) ):- position(P). % representa el coste hasta llegar a la posición P.
...
writeClauses:- 
    startAtCity1,
    endAtCity1,
    oneCityInEachPosition,
    onePositionForEachCity,
    adjacentCities,
    costRoad,
    atmostCost,
    true,!. 
...
startAtCity1:- writeClause( [visited(1, 0)] ).

endAtCity1:- numCities(N), writeClause( [visited(1, N)] ).

oneCityInEachPosition:- city(City), City \= 1, findall(visited(City, P), position(P), Lits), exactly(1, Lits), fail.
oneCityInEachPosition.

onePositionForEachCity:- position(P), findall(visited(City, P), city(City), Lits), exactly(1, Lits), fail.
onePositionForEachCity.

adjacentCities:- adjacency(City1, AdjList), city(City2), position(P1), position(P2), P2 is P1+1, not(member(City2, AdjList)), 
                 writeClause( [-visited(City1, P1), -visited(City2, P2)] ), fail.
adjacentCities.


costRoad:- city(City1), position(P1), city(City2), position(P2), P2 is P1+1, 1 is (City1+City2) mod 2,
           writeClause( [-visited(City1, P1), -visited(City2, P2), cost(P2)] ), fail.
costRoad.

atmostCost:- maxCost(M), findall(cost(P), position(P), Lits), atMost(M,Lits).
atmostCost.
