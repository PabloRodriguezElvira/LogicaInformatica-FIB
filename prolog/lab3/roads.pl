...
writeClauses:- 
    atMostNStations,
    atLeast1GasBetweenCities,
    true, !.

writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

atMostNStations:-
    maxStations(N), findall(install(C), city(C), Lits), atMost(N, Lits), fail.
atMostNStations.

atLeast1GasBetweenCities:-
    city(C1), city(C2), road(C1, C2), atLeast(1, [install(C1), install(C2)]), fail.
atLeast1GasBetweenCities.
...
displaySol(M):- write('Gas stations installed in: '), nl, city(C), member(install(C), M), write(C), nl, fail.
displaySol(_):- nl.
